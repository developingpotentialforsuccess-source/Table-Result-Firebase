...
export default function SettingsModal({ level, levels, onUpdateLevel, onReplaceLevels, onClose, paperStyle, onUpdatePaperStyle, wallpaper, onUpdateWallpaper, gridLineLevel, onUpdateGridLineLevel, settings, onUpdateSettings, onOpenTemplateModal, classRecords, onDeleteAllClasses, activeSyncId }: Props) {
  ...
  const [editingTemplate, setEditingTemplate] = useState<{ id: string, authorId?: string, name: string, levels: Level[], programId?: string } | null>(null);
  ...
  const handleAdminDuplicateTemplate = async (template: any) => {
  ...
  const handleAdminDeleteTemplate = async (templateId: string) => {
    if (!confirm("Are you sure you want to permanently delete this template from the library?")) return;
    try {
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm max-w-4xl mx-auto">
                  ...
                  <LevelSettings 
                    ...
                    onUpdateLevel={(updatedLevel) => {
                      ...
                      // If it has an ID, it's in Firestore, so we can auto-save if admin
                      const isAdmin = ["DPSS", "DPS", "BPS", "BPSS"].includes(unlockCode.trim().toUpperCase());
                      if (isAdmin && editingTemplate.id) {
                        handleAdminUpdateSystemTemplate(editingTemplate.id, [updatedLevel]);
                      }
                    }} 
                  ...
                  <div className="mt-6">
                    {!selectedProgramId ? (
                      ...
                      <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-500">
                          ...
                          savedTemplates.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              ...
                              {(template.levels || []).map((lvl, idx) => {
                                const isAdmin = ["DPSS", "DPS", "BPS", "BPSS"].includes(unlockCode.trim().toUpperCase());
                                return (
                                  <div key={lvl.id || idx} className="space-y-3 p-4 bg-white rounded-3xl border-2 border-slate-100 hover:border-indigo-200 transition-all">
                                    <div className="text-xs text-slate-700 flex items-center justify-between font-bold">
                                      <span className="truncate flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full" style={{ backgroundColor: lvl.color || '#6366f1' }} />
                                        🎒 {lvl.name}
                                      </span>
                                    ...
                                    {isAdmin && (
                                      <div className="flex gap-1 pt-2 border-t border-slate-50">
                                        <button 
                                          onClick={() => setEditingTemplate({ ...template, levels: [lvl] })}
                                          className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 bg-indigo-50 text-indigo-700 rounded-xl text-[10px] font-black uppercase hover:bg-indigo-100 transition-all"
                                        >
                                          <Edit2 className="w-3 h-3" /> Edit Config
                                        </button>
                                        <button 
                                          onClick={() => handleAdminDuplicateTemplate({ ...template, levels: [lvl], name: `${lvl.name} Template` })}
                                          className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 bg-emerald-50 text-emerald-700 rounded-xl text-[10px] font-black uppercase hover:bg-emerald-100 transition-all"
                                        >
                                  ...
                                  <div className="mt-5 pt-4 border-t border-slate-50 grid grid-cols-2 gap-2">
                                    <button 
                                      ...
                                      className="flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-black uppercase tracking-wider text-white bg-gradient-to-r from-pink-500 to-pink-600 rounded-2xl hover:brightness-110 active:scale-95 transition-all shadow-xs shadow-pink-200"
                                    >
                                      <Copy className="w-4 h-4" /> Apply
                                    </button>
                                    <button 
                                      onClick={() => handleDuplicateTemplate(template)}
                                      className="flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-black uppercase tracking-wider text-slate-600 bg-slate-50 rounded-2xl hover:bg-slate-100 hover:text-slate-950 active:scale-95 transition-all"
                                    >
                                    ...
                                    {(isOwner || isAdmin) && (
                                      <button 
                                        ...
                                        className="col-span-2 flex items-center justify-center gap-2 px-4 py-2 text-xs font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-2xl active:scale-95 transition-all"
                                      >
                                ...
                                >
                                  ...
                                  <div className="mt-5 pt-4 border-t border-slate-50 grid grid-cols-2 gap-2">
                                    <button 
                                      ...
                                      className={`flex items-center justify-center gap-2 px-4 py-2.5 text-[10px] font-black uppercase tracking-wider text-white bg-gradient-to-r ${btnBg} rounded-2xl hover:brightness-110 active:scale-95 transition-all`}
                                    >
                                    ...
                                    <button 
                                      ...
                                      className="flex items-center justify-center gap-2 px-4 py-2.5 text-[10px] font-black uppercase tracking-wider text-slate-600 bg-slate-50 rounded-2xl hover:bg-slate-100 hover:text-slate-950 active:scale-95 transition-all"
                                    >
                                    ...
                                    {true && (
                                      <>
                                        <button 
                                          ...
                                          className="flex items-center justify-center gap-2 px-4 py-2 text-[10px] font-black uppercase text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-2xl active:scale-95 transition-all"
                                        >
                                          <Edit2 className="w-3.5 h-3.5" /> Edit
                                        </button>
                                        <button 
                                          onClick={() => handleAdminDuplicateTemplate({
                                            name: `${lvl.name} (Copy)`,
                                            authorName: 'System Admin',
                                            levels: [lvl],
                                            isAdmin: true
                                          })}
                                          className="flex items-center justify-center gap-2 px-4 py-2 text-[10px] font-black uppercase text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-2xl active:scale-95 transition-all"
                                        >
                        ...
                        {/* Shared Sync ID Section */}
                        <div className="mt-8 pt-8 border-t border-slate-200 max-w-md mx-auto text-center">
                          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-50 rounded-full text-[9px] font-black text-blue-500 uppercase tracking-widest mb-4 border border-blue-200/40">
                            <RefreshCw className="w-3 h-3 text-blue-400" />
                            Real-Time Collaboration
                          </div>
                          <h4 className="text-xs font-bold text-slate-800 mb-1">Shared Cloud Sync ID</h4>
                          <p className="text-[10px] text-slate-500 mb-4 font-medium leading-relaxed">To see the same data across different Gmail accounts, enter the same Sync ID on all devices. Leave blank to use your private account.</p>
                          <div className="flex gap-2 max-w-xs mx-auto">
                            <input 
                              ...
                              onChange={(e) => {
                                ...
                                onUpdateSettings({ ...settings, syncId: val });
                                localStorage.setItem("gradecalc_global_sync_id", val);
                              }}
                              placeholder="e.g. DPS_COLLEGE_2026"
                              className="flex-1 px-4 py-2 bg-white border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 focus:outline-none text-center font-mono tracking-widest font-black"
                            />
                          </div>
                          {settings.syncId && (
                            <p className="mt-2 text-[10px] font-black text-blue-600 uppercase tracking-widest animate-pulse">✓ Cloud Sync Active: {settings.syncId}</p>
                          )}
                        </div>
